...
public interface ReceiptDAO {
    ...
    @Query("SELECT SBD.SPLIT_ID AS ACTIVITY_ID,  R.RECEIPT_ID AS RECEIPT_ID, R.RECEIPT_DESCRIPTION AS RECEIPT_DESCRIPTION, R.RECEIPT_DATE AS RECEIPT_DATE, R.GROUP_ID AS GROUP_ID, R.RECEIPT_AMOUNT AS RECEIPT_AMOUNT, US.FIRST_NAME AS SPENDER_FIRST_NAME,US.LAST_NAME AS SPENDER_LAST_NAME, SBD.SPLITTED_AMOUNT AS SPLITTED_AMOUNT, SBD.STATUS AS STATUS, G.GROUP_NAME AS GROUP_NAME FROM RECEIPT AS R INNER JOIN USER AS US ON R.USER_ID = US.USER_ID INNER JOIN SPLIT_BILL_DETAILS AS SBD ON R.RECEIPT_ID = SBD.RECEIPT_ID INNER JOIN GROUPS AS G ON R.GROUP_ID = G.GROUP_ID WHERE SBD.RECEIVER_USER_ID = :userID AND SBD.STATUS = 'settled'")
    LiveData<List<Activity>> filterActivitySettled(Integer userID);

    @Query("SELECT SBD.SPLIT_ID AS ACTIVITY_ID,  R.RECEIPT_ID AS RECEIPT_ID, R.RECEIPT_DESCRIPTION AS RECEIPT_DESCRIPTION, R.RECEIPT_DATE AS RECEIPT_DATE, R.GROUP_ID AS GROUP_ID, R.RECEIPT_AMOUNT AS RECEIPT_AMOUNT, US.FIRST_NAME AS SPENDER_FIRST_NAME,US.LAST_NAME AS SPENDER_LAST_NAME, SBD.SPLITTED_AMOUNT AS SPLITTED_AMOUNT, SBD.STATUS AS STATUS, G.GROUP_NAME AS GROUP_NAME FROM RECEIPT AS R INNER JOIN USER AS US ON R.USER_ID = US.USER_ID INNER JOIN SPLIT_BILL_DETAILS AS SBD ON R.RECEIPT_ID = SBD.RECEIPT_ID INNER JOIN GROUPS AS G ON R.GROUP_ID = G.GROUP_ID WHERE SBD.RECEIVER_USER_ID = :userID AND SBD.STATUS = 'assigned'")
    LiveData<List<Activity>> filterActivityActive(Integer userID);

    @Query("SELECT R.RECEIPT_ID AS RECEIPT_ID, R.RECEIPT_AMOUNT AS AMOUNT, R.RECEIPT_DATE AS DATE, U.FIRST_NAME AS FIRST_NAME, U.LAST_NAME AS LAST_NAME, G.GROUP_NAME AS GROUP_NAME FROM SPLIT_BILL_DETAILS AS SBD INNER JOIN RECEIPT AS R ON R.RECEIPT_ID = SBD.RECEIPT_ID INNER JOIN USER AS U ON U.USER_ID = R.USER_ID INNER JOIN GROUPS AS G ON G.GROUP_ID = R.GROUP_ID WHERE SBD.RECEIVER_USER_ID = :userID ORDER BY R.RECEIPT_DATE DESC LIMIT 8")
    List<DetailedReceiptClass> getDetailedReceiptsByUser(Integer userID);
